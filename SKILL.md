...
### Step 5（可选）：视频录制屏幕内容补全

如果会议有屏幕共享演示内容，在已有文字稿基础上额外提取视觉内容可显著丰富上下文。

**何时建议**（满足任一）：
- 用户提到会议中有演示、PPT 分享、代码展示、白板讨论
- 会议主题涉及产品评审、技术 demo、数据分析展示
- 用户直接提供了本地录制文件路径

若满足条件，询问用户是否提供录制文件路径，然后触发 `video-to-text` skill，将屏幕内容描述追加到已有文字稿末尾。**仅摄像头画面（无共享屏幕讲解）时无需运行。**
...
| `docs +fetch` 返回 403/无权限 | 文档未共享给当前用户 | 提示用户向组织者索取文档共享访问；若无法获取且有录制文件，改走[视频录制内容补全 B 兜底](#视频录制内容补全) |
...
`video-to-text` skill 在 meeting-context 流程中有两种角色：

### A. 主动增强（会议有屏幕共享演示时）

**适用**：已通过 API 获取文字稿，但会议含 PPT/代码 demo/白板/表格讲解等视觉内容。

流程：
1. 在 API 文字稿基础上，额外运行 `video-to-text` 提取屏幕帧
2. 屏幕内容描述（`📺 *屏幕内容*：...`）追加到输出末尾
3. Agent 得到文字 + 视觉双通道上下文

**跳过条件**：仅摄像头画面、无屏幕共享讲解内容——视觉信息对上下文价值低，不建议运行。

### B. 兜底使用（API 全部失败时）

**适用**：`vc +notes` 逐字稿 403（非 owner）且无 `/docx/` 文档链接，或 `docs +fetch` 403 无法解决，或腾讯会议录制（飞书 API 不适用）。

流程：
1. 若用户尚未下载录制，提示下载渠道：
   - 飞书会议：妙记页面 → 「原始录像」
   - 腾讯会议：[会议网页](https://meeting.tencent.com) → 录制管理 → 下载
2. 用户提供本地文件路径后触发 `video-to-text` skill
3. 输出语音逐字稿 + 屏幕内容（若有），作为唯一上下文来源
